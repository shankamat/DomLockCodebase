...
class IntervalCheck{
	public:
		...
		bool IsOverlapModified(interval *shift, interval *parent, int m, int threadID)
		{
			//cout<<"m=1";
			pthread_mutex_lock(&mutex);
			parent->MySeq = ++Seq;
			Array[threadID] = parent;
			pthread_mutex_unlock(&mutex);
			...
			{	
				for(int i=0; i< SIZE; i++)
				{
					...
					{	interval *ptr = Array[i];	
						//wait untill there is an overlap and my sequence number is greater
						while(ptr !=NULL && (ptr->pre <= shift->post && ptr->post >= shift->pre) && ptr->MySeq < parent->MySeq)
						{
							ptr = Array[i];
						}
